      ...
      >
          ...
          <form onSubmit={handleVerify} className="space-y-6 flex-grow">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder={t('verifyCode.form.code.placeholder')}
              />
            </div>
            ...
            >
              {isLoading ? (
                <div className="flex items-center justify-center">
                  ...
                  {t('verifyCode.form.submit.verifying')}
                </div>
              ) : (
                t('verifyCode.form.submit.verify')
              )}
            </motion.button>

            <div className="mt-4 text-center">
              <Link to="/forgot-password" className="text-sm text-[#1B45B4] hover:underline">
                {t('verifyCode.form.backToForgotPassword')}
              </Link>
            </div>
          </form>
        </div>
        ...
        <div className="hidden md:block relative bg-[#1B45B4] text-white flex items-center justify-center overflow-hidden">
          <div className="relative text-center px-10 py-6 z-10">
            <h2 className="text-4xl font-bold mb-6">{t('verifyCode.banner.title')}</h2>
            <p className="text-xl mb-8 leading-relaxed">
              {t('verifyCode.banner.description')}
            </p>
            <div className="grid grid-cols-1 gap-4 mt-8">
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
                <h3 className="font-semibold mb-2">{t('verifyCode.banner.security.title')}</h3>
                <p className="text-sm">{t('verifyCode.banner.security.description')}</p>
              </div>
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
                <h3 className="font-semibold mb-2">{t('verifyCode.banner.next.title')}</h3>
                <p className="text-sm">{t('verifyCode.banner.next.description')}</p>
              </div>
            </div>
          </div>
        </div>
